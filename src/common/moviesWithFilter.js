...
function MovieFilter(){
    ...
    const   handleChange1 = (event) => {
      ...
      const  handleInputChange = () => {
          var copyData = moviesData.filter(   (item)=>{if(MovieName==='' && Genre==='' && Artists!==''){return  (item.artists.some(element => {if(element.id===Artists.id){return true;} return false;}))}
                                                       else if(MovieName==='' && Artists==='' && Genre!==''){return (item.genres.includes(Genre))}
                                                       else if(MovieName!=='' && Artists==='' && Genre===''){return (item.title.includes(MovieName))} 
                                                       else{return  (item.genres.includes(Genre) && item.title.includes(MovieName) && item.artists.some(element => {if(element.id===Artists.id){return true;} return false;}) )}})
        setMoviesData(copyData);
      }
      const hai=(item)=>{
                ...
                <div className="left" >
                    <ImageList >
                        ...
                        {movies_Data.map((item) => (
                            <Link to="/Details" onClick={()=>{hai(item)}}>
                ...
                <div className="filter" >
                    <Card sx={{ maxWidth: 240 }} style={{ float: 'right', padding: '20px', margin: '16px' }}>
                        <CardContent>
                            ...
                            <FormControl>
                                <TextField id="standard-basic" label="Movie Name" onBlur={handleChange0} variant="standard" />
                            </FormControl>
                            <br />
                            <br />
                        ...
                        <CardActions>
                            <Button variant="contained" style={{width:"100%"}} onClick={handleInputChange}>Apply</Button>
                        </CardActions>
                    </Card>
                </div>
            </div>
